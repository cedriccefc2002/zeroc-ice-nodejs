...
export class ArrayUtil {
    public static eq(e1: any, e2: any): boolean {
        if (e1 === e2) {
            return true; // If identity compare equals members are equal.
        } else if (e1 === null || e1 === undefined || e2 === null || e2 === undefined) {
            return false;
            ...
            return ArrayUtil.equals({ lhs: e1, rhs: e2, valuesEqual: ArrayUtil.eq });
        }
        return false;
    }
    ...
    public static equals({ lhs, rhs, valuesEqual }: { lhs: any[] | Uint8Array; rhs: any[] | Uint8Array; valuesEqual?: (v1: any, v2: any) => boolean; }): boolean {
        if (lhs.length !== rhs.length) {
            return false;
        }

        const equalFn = valuesEqual || ArrayUtil.eq;
        for (let i = 0; i < lhs.length; ++i) {
            if (!equalFn.call(equalFn, lhs[i], rhs[i])) {
                return false;
            }
        }
